	     ...
	     unsigned int length)
{
	int i, mod;	
	for (i = 0; i < length; i++) {
		crc32 = (crc32 >> 8) ^ sctp_crc_c[((crc32^buffer[i])&0xff)];
	}
	return (crc32);
}
...
{
	u_int32_t realCRC, crc_parts[4], calc_crc, alt_crc, netways;
	char buffer1[1500];
	int i,at=0x02;
	...
	for(i=0; i < 1000000; i++) {
		realCRC = 0xffffffff;
		realCRC = update_crc32(realCRC,buffer1,4);
		realCRC = sctp_csum_finalize(realCRC);
	}
}
