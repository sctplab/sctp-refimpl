...
main(int argc, char **argv)
{
  ...
  if (binda != NULL) {
	  struct in6_addr in6;
	  struct in_addr in;
	  if (inet_pton(AF_INET6, binda, (void *)&in6)) {
#if 0 /* allow user to do this to test the stack */
		  if (v4only) {
			printf("Error: binding v6 address, but v4only set\n");
			return(-5);
		  }
#endif
		  SCTP_setBind6Addr((u_char *)&in6);
		  bindSpecific = 1;
	  } else if (inet_pton(AF_INET, binda, (void *) &in)) {
#if 0 /* allow user to do this to test the stack */
		  if (v6only) {
			printf("Error: binding v4 address, but v6only set\n");
			return(-5);
		  }
#endif
