...
sendmsg_return:
    /* TODO: Needs a condition for non-blocking when error is EWOULDBLOCK */
    if (0 == error)
        retvalsendmsg = len; /* __Userspace__ How can we determine if all data has been sent? */
    else{
        printf("%s: error = %d\n", __func__, error);
        retvalsendmsg = (-1);
    }
    ...
    *result = 0;
	...
	ptr += sizeof(struct ip);
        if(ip->ip_p == 17) {
             ...
             */
            udp = (struct udp *) ptr;
            ptr += sizeof(struct udp);

            /* TODO what destination port do I send to?  the UDP encap port? which header to i get
             *   the port from?
             */
            
            /* TODO sendto call to userspace_udpsctp, and error handling */
